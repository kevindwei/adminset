...
from cmdb.models import Host as ServerInfor,HostGroup as ServerGroup,Log
...
class webterminal(WebsocketConsumer):
    ...
    def disconnect(self, message):
        ...
        audit_log=Log.objects.get(user=UserInfo.objects.get(username=self.message.user),channel=self.message.reply_channel.name)
            ...
            if text:
                ...
                if data[0] == 'ip':
                    ...
                    try:
                        ...
                        audit_log = Log.objects.create(user=UserInfo.objects.get(username=self.message.user),server=data,channel=self.message.reply_channel.name,width=width,height=height)
                        audit_log.save()
                        if method == 'password':
