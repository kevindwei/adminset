...
from cmdb.models import Credential,Host as ServerInfor,Log
...
class Index(LoginRequiredMixin,PermissionRequiredMixin,TemplateView):
    ...
    login_url = reverse_lazy('admin:login')#返回未通过测试的用户将被重定向到的URL.如果设置了login_url，则返回它,否则返回settings.LOGIN_URL。
